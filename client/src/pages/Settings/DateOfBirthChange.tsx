...
import { useChangeDateOfBirth } from "../../hooks/settings/useChangeDateOfBirth";
import { useProfile } from "../../hooks/user/useProfile";

export const DateOfBirthChange = () => {
	...
	const { changeDateOfBirthMutation } = useChangeDateOfBirth();

	const submit: SubmitHandler<{
		newDate: Date;
		password: string;
	}> = data => {
		changeDateOfBirthMutation.mutate(data);
	};
			...
			<div className="z-10 bg-[#121212] border border-gray-800 rounded-2xl p-10 shadow-2xl w-[900px] flex gap-10">
				<div className="w-1/2">
					...
					<form onSubmit={handleSubmit(submit)} className="flex flex-col gap-3">
						{errors.newDate && (
							<p className="text-red-500 text-xs">{errors.newDate.message}</p>
						)}
						<MyInput
							type="date"
						...
						{changeDateOfBirthMutation.isSuccess && (
