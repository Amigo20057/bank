...
import { useChangePassportNumber } from "../../hooks/settings/useChangePassportNumber";
import { useProfile } from "../../hooks/user/useProfile";

export const PassportNumberChange = () => {
	...
	const { changePassportNumberMutation } = useChangePassportNumber();

	const submit: SubmitHandler<{
		newPassportNumber: string;
		password: string;
	}> = data => {
		changePassportNumberMutation.mutate(data);
	};
			...
			<div className="z-10 bg-[#121212] border border-gray-800 rounded-2xl p-10 shadow-2xl w-[900px] flex gap-10">
				<div className="w-1/2">
					...
					<form onSubmit={handleSubmit(submit)} className="flex flex-col gap-3">
						{errors.newPassportNumber && (
							<p className="text-red-500 text-xs">
								{errors.newPassportNumber.message}
							</p>
						)}
						<MyInput
							type="text"
							placeholder="Passport Number"
							{...register("newPassportNumber", {
						...
						{changePassportNumberMutation.isSuccess && (
